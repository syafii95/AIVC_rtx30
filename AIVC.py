...
class DataHandler_Thread(QThread):
    ...
    def run(self):
        while self.dataHandlerRunning:
            try:
                camSeq, frame, bboxess, formerID, isRasmAnchor= self.yoloResultQue.get(timeout=0.5)
                #print(f'=======> {bboxes} <====') 
            except q.Empty:
                continue
            self.occu.start()
            ...
            #bboxes = utils.postprocess_boxes(bboxes, frame_size, FIXED_INPUT_SIZE, 0.3)
            bboxes = darknet.scaleBboxes(bboxess,FIXED_INPUT_SIZE,frame)
    ...
    def run(self):
        while True:
            ...
            if ret==0:#Sucess read
                ...
                image_resized = cv2.resize(frame, (FIXED_INPUT_SIZE, FIXED_INPUT_SIZE),
                                       interpolation=cv2.INTER_LINEAR)
                self.feedCaptureQue.emit(camSeq, frame, image_resized, formerID, isRasmAnchor)
            else:
...
class Capture_Thread(QThread):
    ...
    def run(self):
        ...
        while self.camThreadRunning:
            ...
            readT=time.time()
            if gloveSensors != -1:  #has plc connection
                ...
                for s in range(CFG.SENSOR_NUM):
                    if gloveSensors[s]:#one former past
                        ...
                        elif self.secPerGloves[s] >3: #Avoid Windows auto adjust time resulting negative value here => 0~0.25s
...
class Inference_Thread(QThread):
    ...
    def feedCaptureQue(self, camSeq, image, image_resized, formerID, isRasmAnchor):
        self.captureQue.put([camSeq, image, image_resized, formerID, isRasmAnchor])
    ...
    def batchProcessYolo(self):
        ...
        for i in range(len(self.payload)):
            self.feedYoloResult.emit(self.payload[i][1],self.payload[i][2],bboxes,self.payload[i][3],self.payload[i][4]) ##let data handler handle
        self.payload.clear()
        self.occu.end()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        config_file = generateYoloCfg(CLASS_NUM)
        self.inferenceThread=Inference_Thread(self,config_file)
...
class DefectionGrid(QWidget):
    ...
    def updateArm(self, index, armID, armRecord, lab='',highlight=False,chain=False):
        gg=armRecord[0]
        rdg=0
        odg=0
        tt=''
            ...
            for index, armID in enumerate(self.armsID):
                ...
                try:
                    ...
                    if(dr <0.2): #Remove from grid
                        ...
                        self.updateArm(index-removedNum, armID, record, highlight=False, chain=True)
