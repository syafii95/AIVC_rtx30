...
def draw_boxes(detections, image, colors, clsLabel):
    import cv2
    for i in detections:
        left, top, right, bottom = bbox2points((i[0],i[1],i[2],i[3]))
        cv2.rectangle(image, (left, top), (right, bottom), colors[i[5]], 2)
...
def scaleBboxes(bboxes,input_size,frame):
    ...
    for i in bboxes:
        ...
        if len(bboxes) > 0:
            scores = [i[4] for i in bboxes]
            best_index = scores.index(max(scores))
            best_bbox = [listBbox[best_index]]
